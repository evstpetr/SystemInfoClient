...
public class WinRegistry {
    ...
    private static final Logger logger = Logger.getLogger(WinRegistry.class);
     ...
     */
    public static String readDWord(String key, String value) throws UnsupportedEncodingException, IOException {
        ...
        try {
            ...
            logger.error("Консоль не запущена", exc);
        }
        ...
        bfOut.write("reg query " + key + " /v " + value);
        bfOut.newLine();
        bfOut.flush();
        ...
        while ((s = bfIn.readLine()) != null) {            
            if (s.contains("0x")){
                bfIn.close();
                bfOut.close();
                s = String.valueOf(Long.parseLong(s.substring(s.indexOf("0x")+2), 16));
                return s;
            }
        }
        return null;
